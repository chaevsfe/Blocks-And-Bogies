...
public abstract class DoubleAxleScotchYokeDisplay implements BogeyDisplay {
    public static class DoubleAxleLargeScotchYoke extends DoubleAxleScotchYokeDisplay {
        ...
        public DoubleAxleLargeScotchYoke(ElementProvider<?> prov) {
            frame = prov.create(BogiePartials.LARGE_4C_FRAME);
            ...
            r_pin = prov.create(BogiePartials.LARGE_4C_R_PIN);
            l_pin = prov.create(BogiePartials.LARGE_4C_L_PIN);
            r_slider = prov.create(BogiePartials.LARGE_4C_R_SLIDER);
            l_slider = prov.create(BogiePartials.LARGE_4C_L_SLIDER);
    ...
    public static class DoubleAxleExtraLargeScotchYoke extends DoubleAxleScotchYokeDisplay {
        private final Affine<?> frame;
        private final Affine<?> belt;
        private final ScrollHandle beltScroll;
        private final Affine<?> r_pin;
        ...
        public DoubleAxleExtraLargeScotchYoke(ElementProvider<?> prov) {
            frame = prov.create(BogiePartials.EXTRA_LARGE_4C_FRAME);
            var scrolling = prov.createScrolling(BogiePartials.EXTRA_LARGE_4P_BELTS, AllSpriteShifts.BOGEY_BELT);
            belt = scrolling.getFirst();
            beltScroll = scrolling.getSecond();
            r_pin = prov.create(BogiePartials.EXTRA_LARGE_4C_R_PIN);
            l_pin = prov.create(BogiePartials.EXTRA_LARGE_4C_L_PIN);
            r_slider = prov.create(BogiePartials.EXTRA_LARGE_4C_R_SLIDER);
            l_slider = prov.create(BogiePartials.EXTRA_LARGE_4C_L_SLIDER);
        ...
        public void update(CompoundTag bogeyData, float wheelAngle) {
            ...
            r_pin.translate(0, 1.25f, 0)
                    .rotateXDegrees(wheelAngle)
                    .translate(0, .375f, 0)
                    .rotateXDegrees(-wheelAngle);

            l_pin.translate(0, 1.25f, 0)
                    .rotateXDegrees(wheelAngle + 90)
                    .translate(0, .375f, 0)
                    .rotateXDegrees(-(wheelAngle + 90));

            r_slider.translate(0, 0, .375f * Math.sin(AngleHelper.rad(wheelAngle)));

            l_slider.translate(0, 0, .375f * Math.sin(AngleHelper.rad(wheelAngle + 90)));
