...
public abstract class TripleAxleTrailingDisplay implements BogeyDisplay {
    public static class TripleAxleSmallTrailing extends TripleAxleTrailingDisplay {
        private final Affine<?> frame;
        private final Affine<?>[] shafts;
        private final Affine<?>[] wheels;

        public TripleAxleSmallTrailing(ElementProvider<?> prov) {
            frame = prov.create(BogiePartials.SMALL_TRAILING_6_FRAME);
            shafts = prov.create(AllPartialModels.SHAFT, 5);
            wheels = prov.create(BogiePartials.SMALL_SHARED_WHEELS, 3);
        }
        ...
        public void update(CompoundTag bogeyData, float wheelAngle) {
            ...
            shafts[3].translate(-.5f, .25f, -.5f)
                    .center()
                    .rotateTo(Direction.UP, Direction.EAST)
                    .rotateYDegrees(wheelAngle)
                    .uncenter();

            shafts[4].translate(-.5f, .25f, -1.5f)
                    .center()
                    .rotateTo(Direction.UP, Direction.EAST)
                    .rotateYDegrees(wheelAngle)
                    .uncenter();
