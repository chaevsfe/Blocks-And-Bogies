...
public abstract class QuintupleAxleBogieDisplay implements BogeyDisplay {
    public static class SmallQuintupleAxleBogie extends QuintupleAxleBogieDisplay {
        private final Affine<?> frame;
        private final Affine<?>[] shafts;
        private final Affine<?>[] wheels;

        public SmallQuintupleAxleBogie(ElementProvider<?> prov) {
            frame = prov.create(BogiePartials.SMALL_BOGIE_10_FRAME);
            shafts = prov.create(AllPartialModels.SHAFT, 4);
            wheels = prov.create(AllPartialModels.SMALL_BOGEY_WHEELS, 5);
        }
        ...
        public void update(CompoundTag bogeyData, float wheelAngle) {
            ...
            shafts[3].translate(-.5f, .25f, -2)
                    .center()
                    .rotateTo(Direction.UP, Direction.SOUTH)
                    .rotateYDegrees(wheelAngle)
                    .uncenter();

            wheels[0].translate(0, .75f, 0)
                    .rotateXDegrees(wheelAngle);

            wheels[1].translate(0, .75f, 1)
                    .rotateXDegrees(wheelAngle);

            wheels[2].translate(0, .75f, -1)
                    .rotateXDegrees(wheelAngle);

            wheels[3].translate(0, .75f, 2)
                    .rotateXDegrees(wheelAngle);

            wheels[4].translate(0, .75f, -2)
                    .rotateXDegrees(wheelAngle);
        }
    }
}
